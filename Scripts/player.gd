...
# Onready Variables
@onready var model = $BeatriceTexture_Player
@onready var animation = $gobot/AnimationPlayer
@onready var animation2 = $BeatriceTexture_Player/AnimationPlayer
...
func perform_jump():
	...
	AudioManager.jump_sfx.pitch_scale = 1.1
	
	jumpTween()
	animation2.play("Jump")
...
func player_animations():
	...
	if is_on_floor():
		if is_moving(): # Checks if player is moving
			animation2.play("Run", 0.5)
			particle_trail.emitting = true
			footsteps.stream_paused = false
		else:
			animation2.play("Char_idle", 0.5)
